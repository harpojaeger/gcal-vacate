...
export default ({
    ...
    instance: SelectableEventInstance;
    handleChange: BooleanFunction;
}) => {
    ...
    if (isAllDayEvent) {
        if (instance.start?.date) {
            formattedStartTime = new Date(
                Date.parse(instance.start.date)
            ).toLocaleDateString();
        }
        if (instance.end?.date) {
            formattedEndTime = new Date(
                Date.parse(instance.end.date)
            ).toLocaleDateString();
        }
    } else {
        if (instance.start?.dateTime) {
            formattedStartTime = new Date(
                Date.parse(instance.start.dateTime)
            ).toLocaleString();
        }
        if (instance.end?.dateTime) {
            formattedEndTime = new Date(
                Date.parse(instance.end.dateTime)
            ).toLocaleString();
        }
    }
    ...
    return (
        <li key={instance.id}>
            ...
            <label htmlFor={instance.id}>
                from {formattedStartTime} to {formattedEndTime}. selected:{' '}
                {instance.selected ? 'yup' : 'nope'}
            </label>
        </li>
    );
};
